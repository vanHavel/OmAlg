...
//Creation of OptParser and setup of supported options
  ...
  optParser.setHelpText("Given an omega semigroup, construct a deterministic weak Buechi automaton "
                            "for its language.");
  ...
  optParser.addFlag("suppress_output", "s", "suppress-output", "Suppress output of warnings by os2w");
  ...
  try {
    ...
    std::cerr << "Type os2w -h for help" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  //construct result
  try {
    omalg::DeterministicBuechiAutomaton A = S->toWeakBuechi();
    ...
    if (outputFile.empty()) {
      ...
      if (!suppressWarnings) {
        std::cerr << "Warning: No output file specified. Writing to stdout. (Type os2w -h for help)";
        std::cerr << std::endl;
      }
    ...
    std::cerr << "Given omega semigroup can not be transformed into equivalent weak Buechi automaton." ;
    std::cerr << std::endl;
    std::cerr << "The language is not weak Buechi recognizable.";
    std::cerr << std::endl;
    return EXIT_FAILURE;
  }
