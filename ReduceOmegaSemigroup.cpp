...
/**
 * Reads an omega semigroup from an input file, and minimize it using the
 * syntactic congruence relation.
...
//Creation of OptParser and setup of supported options
  ...
  optParser.setHelpText("Given an omega semigroup, compute the minimal omega semigroup accepting"
                        "the same language.");
  optParser.setOption("input_file", "i", "input-file", "Path to input file for omega semigroup");
  optParser.setOption("output_file", "o", "output-file", "Path to output file for minimized omega semigroup");
  optParser.addFlag("help", "h", "help", "Print usage text");
  optParser.addFlag("suppress_output", "s", "suppress-output", "Suppress output of warnings by rdos");
  ...
  try {
    ...
    std::cerr << "Type rdos -h for help" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if(inputFile.empty()) {
    ...
    if(!suppressWarnings) {
      std::cerr << "Warning: No input file specified. Reading from stdin. (Type rdos -h for help)";
      std::cerr << std::endl;
    }
    //Reading from stdin
    try {
  ...
  if (outputFile.empty()) {
    ...
    if (!suppressWarnings) {
      std::cerr << "Warning: No output file specified. Writing to stdout. (Type rdos -h for help)";
      std::cerr << std::endl;
    }
